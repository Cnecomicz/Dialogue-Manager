...
def test_starting_current_vertex(hello_world_graph, mock_game_state):
    graph_navigator = GraphNavigator(hello_world_graph, mock_game_state)
...
def test_has_valid_edges(hello_world_graph, mock_game_state):
    ...
    assert graph_navigator.current_edges == {
        "vertex_0_to_buy_flower": hello_world_graph.edge_dict["vertex_0_to_buy_flower"],
        "vertex_0_to_end": hello_world_graph.edge_dict["vertex_0_to_end"]
    }
...
def test_error_when_selecting_invalid_edge(hello_world_graph, mock_game_state):
    ...
    with raises(InvalidEdgeError):
        graph_navigator.select("vertex_0_to_insufficient_gold")
...
def test_select_edge_to_cause_effects(hello_world_graph, mock_game_state):
    graph_navigator = GraphNavigator(hello_world_graph, mock_game_state)
    graph_navigator.select("vertex_0_to_buy_flower")
...
def test_select_edge_to_go_to_next_vertex(hello_world_graph, mock_game_state):
    graph_navigator = GraphNavigator(hello_world_graph, mock_game_state)
    graph_navigator.select("vertex_0_to_buy_flower")
    assert graph_navigator.current_vertex == "buy_flower"
...
def test_get_valid_edges(hello_world_graph, mock_game_state):
    graph_navigator = GraphNavigator(hello_world_graph, mock_game_state)
    assert graph_navigator.get_current_edges() == ["vertex_0_to_buy_flower", "vertex_0_to_end"]
...
def test_edge_predicate_1_and_predicate_2(hello_world_graph, mock_game_state_with_limited_inventory):
    graph_navigator = GraphNavigator(hello_world_graph, mock_game_state_with_limited_inventory)
    graph_navigator.select("vertex_0_to_buy_flower")
    with raises(InvalidEdgeError):
        graph_navigator.select("buy_flower_to_buy_flower")
    graph_navigator.select("buy_flower_to_insufficient_flowers")